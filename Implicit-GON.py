...
dataset_name = 'GTZAN'
sr = 22050
img_size = 10 * sr  # I THINK length * sr
n_channels = 1  # Output shape. 1 for images (v) 1 for sound (a)
img_coords = 1  # Input shape. 2 for images (x, y) 1 for sound (t)
...
class SirenLayer(nn.Module):
    ...
    # Is this right? b = 1 / fan_in          for first
    #                b = sqrt(6/fan_in) / 30 else
    # Where has this come from? NOT in SIREN paper
    ...
    # Interesting: no bias! Does this make a difference?
...
def get_mgrid(sidelen, dim=2):
    ...
    return mgrid
# I think this just generates the co-ordinate set.
...
def collate_fn(batch_data, max_len=sr*30):
    audio = batch_data[0][0]
...
if dataset_name == 'GTZAN':
    dataset = torchaudio.datasets.GTZAN('C:/Users/pgc8/anaconda3/Lib/site-packages/torchaudio/datasets/data', download=True, subset='training')

train_loader = torch.utils.data.DataLoader(dataset, sampler=None, shuffle=True, collate_fn=collate_fn, batch_size=batch_size, drop_last=True)
...
c = torch.stack([get_mgrid(img_size, 1) for _ in range(batch_size)]).to(device) # coordinates
...
for step in range(401):
    ...
    if step % 100 == 0 and step > 0:
        ...
        # save file
        torchaudio.save(f'audio/sample_{step}.wav', g, sr)
